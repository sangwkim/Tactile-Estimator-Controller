...
//            = (N^-1 * C)^-1 * (O^-1 * C)
// Encourages that the contact line with respect to (Resting / Equilibrium) Object Poses are parallel to each other
// In other words, encourages even penetration

namespace gtsam_custom_factors {

class PenEven: public gtsam::NoiseModelFactor3<gtsam::Pose3, gtsam::Pose3, gtsam::Pose3> {

public:

  PenEven(gtsam::Key key1, gtsam::Key key2, gtsam::Key key3,
