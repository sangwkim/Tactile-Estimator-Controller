...
if show_viz:
    viz = visualization(height, pose_rel, height, simulator.T_og, view_elev=0, view_azim=230, view_center=(0,0,-height), view_radius=30, env_type='floor')
    viz.set_show(env=False, grp=True, obj_gt=True, obj_est=True, cpoint=True, cline=False)

# Generate Command Trajectory
com_rot_queue = []
theta = np.linspace(0, 5.65*np.pi, 500)
t = theta * (1+theta**2)**0.5 + np.log((1+theta**2)**0.5 + theta)
f = interp1d(t, theta)
t_ = np.linspace(0, t[-1], 180)
theta_ = f(t_)
...
for i in range(50):
    com_rot_queue.append(R.from_euler('zyz', [theta_[-1], beta*theta_[-1]+0.01*i, -theta_[-1]]).as_matrix())
